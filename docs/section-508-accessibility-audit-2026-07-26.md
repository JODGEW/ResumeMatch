...
All five representative pages score **100/100** on the Lighthouse accessibility audit, measured on 2026-07-26 against the tree committed as `58fe0a0`. Automated checks are complemented by the manual verification listed below, since automated tooling covers only a subset of WCAG success criteria. Full Lighthouse reports (HTML + JSON) are archived alongside this document; see **Evidence archive** below for what the archive does and does not cover.
...
**The artifacts were captured against the tree committed as `58fe0a0`, which is no longer HEAD.** Every run finished between 06:27 and 06:37 UTC on 2026-07-26; `58fe0a0` was committed at 06:44 UTC the same morning. Two later commits, `10e8166` (skip links for 2.4.1, neutral empty-state icon) and `78693fc` (error-banner copy), have since changed 8 files under `src/`, including `src/index.css`, `src/components/Layout.tsx`, `src/components/LegalLayout.tsx`, `src/pages/Landing.tsx` and `src/pages/Results.tsx`. Lighthouse has not been re-run since. The 100/100 scores above are therefore accurate for `58fe0a0` and are not direct evidence about HEAD; the skip-link change in particular affects an audited criterion (2.4.1) and its effect on the automated score is unmeasured.
...
| 1.4.3 Contrast (Minimum) | AA | Supports | Text color tokens verified ≥ 4.5:1 against their surfaces in both light and dark themes (remediated 2026-07-26). Pre-remediation evidence is archived: `pre-remediation/login.json` records the sign-in divider label "or sign in with email" (`main.auth-main > div.auth-box > div.auth-divider > span`) at 3.45:1, foreground `#62666d` on background `#08090a` at 12.5px, against a 4.5:1 requirement. The post-fix `/login` run reports no `color-contrast` finding. See the note on citation strength in the remediation log. **⚠ Contradicted by measurement, 2026-07-27, status not yet revised.** The claim above says "both light and dark themes". The 2026-07-27 light-theme run fails `color-contrast` on three of the five audited routes: landing 25 elements, sign-in 3, sample report 16. The recurring pairs are `--accent` `#5e6ad2` on `--surface-inset` `#f7f8f8` (4.41:1), on `#eff0fb` (4.14:1) and on `#e8eaf4` (3.91:1); light `--text-tertiary` `#6c7482` on `#f7f8f8` (4.42:1), a token whose own comment in `src/index.css` claims "≥4.5:1 on white card surfaces" and which does hold on white but not on the inset surface; light `--success` `#178753` on `#f7f8f8` (4.26:1); and `--success-alt` `#0e9668` on `#ffffff` (3.76:1) and `#f3faf7` (3.55:1). This is not a regression: `58fe0a0` audited in light fails the same way. The dark theme passes on all five routes in every run. Revising this row to Partially Supports is a conformance decision that has not been made. |
