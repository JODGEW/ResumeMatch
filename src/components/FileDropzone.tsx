...
interface Props {
  ...
  /**
   * Show the mascot instead of the upload icon. Upload.tsx passes this only on
   * first use (no saved resume) so returning users are not greeted again.
   */
  showMascot?: boolean;
}
...
export function FileDropzone({ onFileSelect, showMascot = false }: Props) {
  ...
  return (
    <>
      ...
      >
        ...
        {showMascot ? (
          <div className="dropzone__mascot" aria-hidden="true">
            <Mascot state={isDragActive ? 'success' : 'idle'} size={96} />
          </div>
        ) : (
